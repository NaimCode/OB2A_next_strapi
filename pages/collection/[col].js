...
const Col = ({ col }) => {
  ...
  return (
    <div className="py-[60px]">
      ...
      <div className="w-full h-[230px] relative group overflow-hidden ">
      ...
      <h3 className="m-auto max-w-[600px] text-xl text-gray-600 py-6 px-2  text-center">
        {cat.description}
      </h3>
      <div className="container mx-auto px-5">
        {produits.length === 0 ? (
          <h2 className="my-16 mx-auto text-center text-gray-400">
            Pas de produit pour cette collection pour l'instant
          </h2>
        ) : (
...
export async function getStaticPaths() {
  const cats_res = await fetch(`${API_URL}/collections`);
...
export async function getStaticProps({ params }) {
  const cat_res = await fetch(`${API_URL}/collections?slug=${params.col}`);
